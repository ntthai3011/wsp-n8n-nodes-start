...
function getAssertionNodes(node) {
  var _a, _b, _c, _d;
  if (node.typeAnnotation.type === import_utils.AST_NODE_TYPES.TSTypeReference && node.typeAnnotation.typeName.type === import_utils.AST_NODE_TYPES.Identifier && node.typeAnnotation.typeName.name === "NodePropertyTypes" && ((_a = node.parent) == null ? void 0 : _a.type) === import_utils.AST_NODE_TYPES.Property && node.parent.key.type === import_utils.AST_NODE_TYPES.Identifier && node.parent.key.name === "type") {
    const insertionNode = (_d = (_c = (_b = node.parent) == null ? void 0 : _b.parent) == null ? void 0 : _c.parent) == null ? void 0 : _d.parent;
    if (!insertionNode)
      return null;
    if ("key" in insertionNode && "type" in insertionNode.key && insertionNode.key.type === import_utils.AST_NODE_TYPES.Identifier && insertionNode.key.name === "properties") {
      return {
