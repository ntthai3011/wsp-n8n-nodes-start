...
const getInsertionArgs = (referenceNode) => {
  if (referenceNode.ast.type === import_utils.AST_NODE_TYPES.PropertyDefinition || referenceNode.ast.type === import_utils.AST_NODE_TYPES.ClassProperty) {
    return {
...
function keyValue(key, value, { backtickedValue } = { backtickedValue: false }) {
  const unescapedQuote = new RegExp(new RegExp("(?<!\\\\)'"), "g");
